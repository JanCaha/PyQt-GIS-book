...
format:
  html: 
    code-tools:
    #   source: file://~/OneDrive/Quarto/App/app1/app.py 
        source: true
---
...
:::{.qgis}
Jako samostatná okna fungují například _Atributová tabulka_, nebo _Rozvžení_ (_Layout_) v **QGIS**.
:::

Základním stavebním prvkem GUI v [PyQt]{.sw} je {{< qt-class QWidget >}}, tento objekt je základem všech prvků uživatelského rozhraní a definuje obecné a pro všechny prvky GUI společné chování, nastavení a možnosti. Později si také ukážeme, jak lze na základě této třídy vytvářet vlastní prvky GUI. 
<!---
...
:::{.qgis}
V QGISu je hlavním widgetem mapové pole (třída `QgsMapCanvas`) spolu s několika drobnými widgety pod mapovým polem, kolem kterého jsou umístěně dokovatelné widgety [Prohlížeč]{.settings}, [Vrstvy]{.settings} a volitelně další, např. [Nástroje zpracování]{.settings}.
:::
...
Timto se nám povedlo vytvořit minimalistickou GUI aplikaci. Její funkcionalita je značně omezená, ale lze na ní vhodně demonstrovat důležité základní koncepty tvorby aplikací s použítím **PyQt**.
